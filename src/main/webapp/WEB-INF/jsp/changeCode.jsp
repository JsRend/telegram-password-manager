...
<head>
    ...
    <script>
        ...
        function press(num) {
            ...
            if (input.length === 5 && (firstTryCode == 0)) {

                firstTryCode = input.join('');
                alert('Введите новый код-пароль ещё раз');
                input = [];
                updateDots();

            } else if (input.length === 5 && (firstTryCode != 0) && (secondTryCode == 0)) {


                secondTryCode = input.join('');

                if (firstTryCode == secondTryCode) {
                    alert('Новый код-пароль установлен!');
                    const form = document.getElementById("form");

                    document.getElementById("userId").value = userId;
                    document.getElementById("userCode").value = secondTryCode;

                    form.submit();

                } else {
                    alert('Введённые код-пароли не совпадают!');
                    input = [];
                    firstTryCode = [];
                    secondTryCode = [];
                    updateDots();
...
<body>
    ...
    <form action="${pageContext.request.contextPath}/change-code-password" id="form" method="post">
        <input type="hidden" name="userCode" id="userCode" required>
        <input type="hidden" name="userId" id="userId" required>
    </form>

    <div align="center" class="lock-icon">
        <i class="fas fa-lock" style="color: #FFD43B;"></i>
        <div class="title">Создайте новый код-пароль</div>
    </div>
    ...
    <a href="javascript:Telegram.WebApp.HapticFeedback.impactOccurred('medium');">
        <div class="numpad">
            ...
            <div href="javascript:Telegram.WebApp.HapticFeedback.impactOccurred('medium');" class="key" onclick="press(1)">1</div>
